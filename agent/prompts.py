...
# AGENT CAPABILITIES
- **`get_files_info(directory: str = ".", **kwargs) -> str: Use this to list files and directories. Useful for exploring the file system and understanding the project structure.
- **`run_python_file(
    file_path: str, args: list[str] | None = None, working_directory: str = ".", **kwargs
) -> str: Use this to execute Python scripts. Provide `args` if the script requires command-line arguments.
- **`write_file(file_path: str, content: str, **kwargs) -> str: Use this to create new files or overwrite existing ones. Be careful when overwriting, and always confirm with the user if it's a critical file.
- **`get_file_content(file_path: str, **kwargs) -> str: Use this to read the content of a file. Essential for understanding code, configuration, or data.
